...
class QueryRequest(BaseModel):
    ...
    max_tokens: int = Field(2048, ge=1, le=4096, description="Maximum tokens in response")
    temperature: float = Field(0.7, ge=0.0, le=1.0, description="LLM temperature")
    top_k_retrieval: int = Field(5, ge=1, le=20, description="Number of documents to retrieve")
...
class ResponseMetadata(BaseModel):
    ...
    retrieval_time_ms: Optional[int] = Field(None, description="Time spent on retrieval")
    inference_time_ms: Optional[int] = Field(None, description="Time spent on inference")
