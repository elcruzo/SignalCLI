...
class MetricsCollector:
    ...
    def record_query_metrics(
        ...
        self.increment_counter(
            "queries", labels={"status": "success" if success else "error"}
        )

        if success:
            # Record latency
            self.record_histogram(
                "query_latency_ms", latency_ms, labels={"model": model_name}
            )

            # Record tokens
            self.increment_counter(
                "tokens_processed", tokens_used, labels={"model": model_name}
            )
